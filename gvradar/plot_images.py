...
def plot_fields_PPI(radar, sweep=0, fields=['CZ'], max_range=150, png=False, outdir='', add_logos=True):
    ...
    for index, field in enumerate(fields):
        ...
        units,vmin,vmax,cmap,title = get_field_info(radar, field)
        ...
        annotate_plot(display, radar_lat, radar_lon, max_range, ax, add_logos, fig, num_fields, nrows, ncols)
    ...
    if num_fields >= 2:
        plt.suptitle(mytitle,fontsize = 9*ncols, weight ='bold', y=(1.0+(ncols*0.03)))
...
def annotate_plot(display, radar_lat, radar_lon, max_range, ax, add_logos, fig, num_fields, nrows, ncols):
    #plt.yticks(rotation=90, va = 'center')
    ...
    if add_logos:
        ...
        if num_fields < 2:
            ...
            #imageboxnasa = OffsetImage(nasalogo, zoom=0.18)
            #imageboxgpm = OffsetImage(gpmlogo, zoom=0.10)
            imageboxnasa = OffsetImage(nasalogo, zoom=0.045*ncols)
            imageboxgpm = OffsetImage(gpmlogo, zoom=0.025*ncols)
            imageboxnasa.image.axes = fig
            imageboxgpm.image.axes = fig
            #abnasa = AnnotationBbox(imageboxnasa,[0,0], xybox=[175, 727],
            abnasa = AnnotationBbox(imageboxnasa,[0,0], xybox=[43.75*ncols, 363.55*nrows],
                                    xycoords= 'figure points', boxcoords='figure points',
                                    pad=0.0, frameon=False)
            #abgpm = AnnotationBbox(imageboxgpm,[0,0], xybox=[1475, 732],
            abgpm = AnnotationBbox(imageboxgpm,[0,0], xybox=[368.75*ncols, 366*nrows],
                                   xycoords= 'figure points', boxcoords='figure points',
                                   pad=0.0, frameon=False)
