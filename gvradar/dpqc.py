...
def unfold_phidp(self):
    ...
    # STEP 0: Estimate system phase offset
    ...
    sample_values = []
    for iray in range(0, nrays, max(1, nrays // 100)):  # Sample every ~100th ray
        for igate in range(sample_start_gate, min(sample_end_gate, ngates)):
            ...
            if val != BAD_DATA and val >= 0 and val <= 360:
                ...
                if ref_val > 10:  # Only where there's precipitation
                    sample_values.append(val)
                    break  # Only need first valid gate per ray
    ...
    # STEP 1: Remove unrealistic PhiDP values based on range (ACCOUNTING FOR SYSTEM PHASE)
    ...
    min_expected_phidp = system_phase - 20.0  # Allow 20° below system phase
